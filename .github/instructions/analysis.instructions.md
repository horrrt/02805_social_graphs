...
- Write every number the page quotes to the script's JSON in `analysis/` or `docs/`. The page reads it
  from there.
- When a page script reads a new field, add it to that week's model (`analysis/week01_schemas.py` to `analysis/week04_schemas.py`), and
  run `python analysis/check_pages.py`. Week 4 scripts also call `check(path, data)` before writing.
